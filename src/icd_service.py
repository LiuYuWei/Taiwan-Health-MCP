...
class ICDService:
    ...
    def _initialize_database(self):
        ...
        try:
            ...
            sheet_cm = next((s for s in xls.sheet_names if "CM" in s and "刪除" not in s), None)
            if sheet_cm:
            ...
            sheet_pcs = next((s for s in xls.sheet_names if "PCS" in s and "刪除" not in s), None)
            if sheet_pcs:
    ...
    def infer_complications(self, code: str) -> str:
        ...
        return json.dumps({
            "base_code": code,
            "potential_complications_or_specifics": children
        }, ensure_ascii=False)
    ...
    def get_conflict_info(self, diagnosis_code: str, procedure_code: str) -> str:
        ...
        """
        try:
            diag = self._query_db("SELECT * FROM diagnoses WHERE code = ?", (diagnosis_code,))
            proc = self._query_db("SELECT * FROM procedures WHERE code = ?", (procedure_code,))
