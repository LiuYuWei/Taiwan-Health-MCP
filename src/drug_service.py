...
class DrugService:
    def __init__(self, data_dir: str):
        ...
        self.scheduler.add_job(self._update_all_data, 'cron', day_of_week='tue', hour=0, minute=0)
    ...
    def _download_and_insert(self, url, table_name, conn, columns_map, pk_col=None):
        ...
        try:
            ...
            if "license_id" in cols:
                cursor.execute(f"CREATE INDEX IF NOT EXISTS idx_{table_name}_lid ON {table_name}(license_id)")
    ...
    def search_drug(self, keyword: str):
        ...
        """
        if not os.path.exists(self.db_path): return "DB initializing..."
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        ...
        if not rows: return f"No results found for '{keyword}'."
        
        results = []
        for r in rows:
            results.append(f"[{r[3]}] {r[0]} ({r[1]})\n   類別: {r[4]}\n   適應症: {r[2][:100]}...") # Truncate indication if too long
    ...
    def get_details(self, license_id: str):
        ...
        # 2. Get Ingredients
        cursor.execute("SELECT ingredient_name, content, unit FROM ingredients WHERE license_id = ?", (license_id,))
        ingredients = cursor.fetchall()
        
        # 3. Get Appearance
        cursor.execute("SELECT shape, color, marking, image_url FROM appearance WHERE license_id = ?", (license_id,))
        app = cursor.fetchone()
        
        # 4. Get Documents
        cursor.execute("SELECT insert_url FROM documents WHERE license_id = ?", (license_id,))
        doc = cursor.fetchone()

        conn.close()
        
        # Format Output
        ing_list = ", ".join([f"{i['ingredient_name']} {i['content']}{i['unit']}" for i in ingredients])
...
"""
    ...
    def identify_pill(self, features: str):
        ...
        if not rows: return "No matching pills found based on description."
        
        return "\n".join([f"藥名: {r[0]} ({r[1]})\n   特徵: {r[3]} {r[2]} (刻痕: {r[4]})" for r in rows])